...
<body>
  ...
  <main>
    ...
    <section class="card">
      <h2 id="episode-title">选择一个 Episode</h2><div id="episode-meta" class="meta"></div><div id="episode-messages" class="messages"><div class="empty">Episode 的脱敏证据会显示在这里。</div></div>
    </section>
  </main>
  <script>
    ...
    async function loadEpisode(id) { const detail=await api('/api/v1/episodes/'+encodeURIComponent(id)); byId('episode-title').textContent=detail.title; byId('episode-meta').textContent=`${detail.source_type} · ${detail.pipeline_version} · ${detail.extraction_status}`; const root=byId('episode-messages'); root.replaceChildren(); for(const item of detail.messages){ const box=node('article',undefined,`message ${item.role||''} ${item.sensitivity||''}`); box.append(node('strong',`${item.role||'unknown'} · evidence ${item.position+1}`),node('div',item.redacted_text===null?'[无安全文本]':item.redacted_text)); root.append(box); } }
    async function refresh() { try { const source=byId('source').value; const query=source?'?limit=100&source_type='+encodeURIComponent(source):'?limit=100'; const [sessions,jobs,episodes]=await Promise.all([api('/api/v1/sessions'+query),api('/api/v1/jobs?limit=10'),api('/api/v1/episodes'+query)]); renderSessions(sessions); renderJobs(jobs.items); renderEpisodes(episodes); } catch(error) { byId('upload-status').textContent='加载失败：'+error.message; } }
    ...
    byId('refresh').addEventListener('click',refresh); byId('source').addEventListener('change',refresh); byId('upload').addEventListener('click',upload); byId('rebuild-episodes').addEventListener('click',rebuildEpisodes); refresh();
  </script>
</body>
</html>"""
